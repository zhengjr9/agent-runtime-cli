...
function buildHeaders(config: AppConfig, session: SessionRecord): Record<string, string> {
  return {
    ...
    'User-Agent': 'claude-a2a-tui/0.1.0',
  ...
  while (!done) {
    ...
    const next = await Promise.race([
      ...
      new Promise<{ idle: true }>((resolve) => {
        const timeout = setTimeout(() => resolve({ idle: true }), sawAssistantText ? 1500 : 30000)
    ...
    if ('idle' in next) {
      ...
      throw new Error('A2A stream timed out before any assistant output')
    }
