...
export async function saveSessionBundle(
  ...
  const updated = {
    ...
    lastPreview: previewMessage(bundle.messages.at(-1)).slice(0, 120),
  }
...
export function newToolUseMessage(input: {
  toolUseId: string
  toolName: string
  toolArgs?: unknown
  toolCallId?: string
  ...
  return {
    ...
    toolDescribe: input.toolDescribe,
  }
}

export function newToolResultMessage(input: {
  toolUseId: string
...
function previewMessage(message: MessageRecord | undefined): string {
  if (!message) {
    return ''
  }
