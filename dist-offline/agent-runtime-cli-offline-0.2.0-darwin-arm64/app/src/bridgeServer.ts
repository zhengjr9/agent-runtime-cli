...
function initMessage(): Record<string, unknown> {
  return {
    ...
    slash_commands: ['clear', 'compact', 'help', 'exit'],
  }
}
...
server.listen(port, host, () => {
  console.log(`Claude A2A bridge listening on http://${host}:${port}`)
  console.log(`Connect Claude-Code with: claude connect http://${host}:${port}`)
})
