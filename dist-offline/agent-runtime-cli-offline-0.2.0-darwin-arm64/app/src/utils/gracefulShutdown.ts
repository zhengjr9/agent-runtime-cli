...
function clearRecentInteractiveLines(lineCount: number): void {
  if (!process.stdout.isTTY || lineCount <= 0) {
    return
  }

  let output = '\r' + ERASE_LINE
  for (let i = 1; i < lineCount; i += 1) {
    output += cursorUp(1) + ERASE_LINE
  }
  if (lineCount > 1) {
    output += cursorDown(lineCount - 1)
  }
  output += CURSOR_LEFT
  writeSync(1, output)
}
...
export async function gracefulShutdown(
  ...
  if (options?.clearPromptLine) {
    try {
      clearRecentInteractiveLines(3)
    } catch {
      // stdout may be closed. Ignore write errors.
    }
  }
